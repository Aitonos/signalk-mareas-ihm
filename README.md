...
- **Pablo** (OpenPlotter testing partner) — reported that `navigation.anchor.position` was being re-emitted as `null` every ~6 s while the boat was not anchored, causing `@meri-imperiumi/signalk-autostate` (and any other subscriber) to interpret each null delta as an "anchor just lifted" event and flip the boat state from "moored" to "underway" continuously. Precise diagnosis pointed straight at the culprit. Fix in 2.10.2: canonical anchor paths only emitted on state transitions (drop/lift), then silence. Thank you.
...
- **Pablo** (partner de testing OpenPlotter) — reportó que `navigation.anchor.position` se re-emitía como `null` cada ~6 s mientras el barco no estaba fondeado, causando que `@meri-imperiumi/signalk-autostate` (y cualquier otro suscriptor) interpretara cada delta null como un evento "acaban de levar" y el estado del barco pasase de "amarrado" a "navegando" continuamente. Diagnóstico preciso que apuntó directo al culpable. Fix en 2.10.2: paths canónicos del ancla emitidos solo en transiciones de estado (drop/lift), después silencio. Gracias.
