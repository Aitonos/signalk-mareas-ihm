...
- 🚢 **Triple online AIS engine** (aisstream.io + aishub.net + aisfriends.com) — three independent optional feeds to configure in any combination when you have no VHF AIS receiver, or to extend VHF coverage. Symmetric dedup priority: VHF ▶ aisstream (real-time WebSocket) ▶ aishub/aisfriends (peer polling 1/min). Every accepted target is republished to the SignalK `vessels.*` tree so any consumer in your environment sees them alongside your VHF ones. Source badge (VHF/AS/AH/AF) shown next to each vessel name in the AIS list.
...
- **[@ABS0lute-1](https://github.com/ABS0lute-1)** — reported [issue #27](https://github.com/Aitonos/signalk-mareas-ihm/issues/27): the wave direction arrow on the overview page always pointed from the North even though the Shelter forecast popup showed the correct direction; Ctrl+F5 rendered it right for about half a second before snapping back. The precise "half a second correct, then wrong" observation pointed straight at a polling callback overwriting a good value. Root cause was a missing type check that let `null` from the on-board wave engine (when it rejects due to no motion) silently overwrite the Open-Meteo forecast. Fixed in 2.10.4 by extracting a single `_effectiveWaveDirDeg` helper used by both the overview map overlay and the Shelter popup, so the two views stay coherent by construction. Thank you.
- **Pablo** (OpenPlotter testing partner) — two reports on consecutive days, both diagnosed with precision that pointed straight at the culprit:
  1. `navigation.anchor.position` was being re-emitted as `null` every ~6 s while the boat was not anchored, causing `@meri-imperiumi/signalk-autostate` (and any other subscriber) to interpret each null delta as an "anchor just lifted" event and flip the boat state from "moored" to "underway" continuously. Fixed in 2.10.2: canonical anchor paths only emitted on state transitions (drop/lift), then silence.
  2. Follow-up: 2.10.2 still emitted a single `null` at plugin startup (our state flag was `null` on boot and got treated as needing a clear), and Pablo also reported that `environment.tide.tendency` had unexpectedly switched from `"Subiendo"` to `"Rising"` — the value tracked the UI language of the webapp instead of being a stable SK canonical enum. Both fixed in 2.10.3: startup silent on the canonical bus, and tide paths (`tendency`, `flow.strength`, `flow.direction`) publish canonical enums (`rising`/`falling`/`slack`, `gentle`/`moderate`/`strong`, `flood`/`ebb`/`slack`) with new `*Localized` sibling paths for dashboards that want the human string. Thank you.
...
- 🚢 **Motor AIS online triple** (aisstream.io + aishub.net + aisfriends.com) — tres feeds independientes opcionales para combinar como quieras cuando no tienes receptor AIS por VHF, o para extender la cobertura del VHF. Dedupe simétrica con prioridad: VHF ▶ aisstream (WebSocket en tiempo real) ▶ aishub/aisfriends (peer polling 1/min). Cada target aceptado se republica al árbol `vessels.*` de SignalK para que cualquier consumidor de tu entorno lo vea junto a los del VHF. Badge de fuente (VHF/AS/AH/AF) detrás del nombre en el listado AIS.
...
- **[@ABS0lute-1](https://github.com/ABS0lute-1)** — reportó el [issue #27](https://github.com/Aitonos/signalk-mareas-ihm/issues/27): la flecha de dirección de olas en el visor de fondeo siempre apuntaba viniendo del Norte, aunque el popup de Previsión de Abrigo mostraba la dirección correcta; Ctrl+F5 la dibujaba bien durante medio segundo y luego saltaba al Norte. La observación precisa de "medio segundo bien y luego mal" apuntó directamente a un callback de polling pisando un valor bueno. Causa raíz: faltaba un type check y un `null` del motor de olas del barco (cuando rechaza por no haber movimiento) sobreescribía silenciosamente el forecast de Open-Meteo. Arreglado en 2.10.4 extrayendo un único helper `_effectiveWaveDirDeg` usado tanto por el overlay del mapa como por el popup Shelter, de modo que ambas vistas quedan coherentes por construcción. Gracias.
- **Pablo** (partner de testing OpenPlotter) — dos reportes en días consecutivos, ambos diagnosticados con precisión que apuntó directo al culpable:
  1. `navigation.anchor.position` se re-emitía como `null` cada ~6 s mientras el barco no estaba fondeado, causando que `@meri-imperiumi/signalk-autostate` (y cualquier otro suscriptor) interpretara cada delta null como un evento "acaban de levar" y el estado del barco pasase de "amarrado" a "navegando" continuamente. Arreglado en 2.10.2: paths canónicos del ancla emitidos solo en transiciones de estado (drop/lift), después silencio.
  2. Follow-up: 2.10.2 seguía emitiendo un único `null` al arrancar el plugin (nuestro flag de estado valía `null` al boot y se trataba como "necesita limpieza"), y Pablo también reportó que `environment.tide.tendency` había pasado inesperadamente de `"Subiendo"` a `"Rising"` — el valor seguía el idioma UI de la webapp en vez de ser un enum canónico SK estable. Ambos arreglados en 2.10.3: arranque silencioso en el bus canónico, y los paths de marea (`tendency`, `flow.strength`, `flow.direction`) publican enums canónicos (`rising`/`falling`/`slack`, `gentle`/`moderate`/`strong`, `flood`/`ebb`/`slack`) con nuevos paths hermanos `*Localized` para dashboards que quieran el string humano. Gracias.
